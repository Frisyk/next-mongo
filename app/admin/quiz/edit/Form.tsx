'use client';
import { putQuiz } from '@/lib/action';
...
export default function EditQuizForm({ m }: { m: string }) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const result = await putQuiz({ id: quiz._id }, formData);
    ...
    <div className="min-h-screen bg-gray-100 p-6 flex items-center w-full justify-center">
      <div className="bg-white shadow-md rounded-lg p-6 w-full">
        ...
        <form onSubmit={handleSubmit}>
          <input type="hidden" name="id" defaultValue={quiz._id} />
          ...
          {quiz.answers.map((answer: any, index: number) => (
            <div key={index} className="mb-4">
              <label className="block text-gray-700 text-sm font-bold mb-2">Option {String.fromCharCode(65 + index)}</label>
              <input
                type="text"
